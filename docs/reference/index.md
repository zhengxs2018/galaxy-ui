# Galaxy UI

## 接口

- [ConfigContext](interfaces/ConfigContext.md)
- [SizeContext](interfaces/SizeContext.md)

## 类型别名

- [ButtonEmits](type-aliases/ButtonEmits.md)
- [ButtonNativeType](type-aliases/ButtonNativeType.md)
- [ButtonProps](type-aliases/ButtonProps.md)
- [ButtonType](type-aliases/ButtonType.md)
- [ButtonVariantType](type-aliases/ButtonVariantType.md)
- [ComponentSize](type-aliases/ComponentSize.md)

## 变量

- [BUTTON\_NATIVE\_TYPES](variables/BUTTON_NATIVE_TYPES.md)
- [BUTTON\_TYPES](variables/BUTTON_TYPES.md)
- [BUTTON\_VARIANT\_TYPES](variables/BUTTON_VARIANT_TYPES.md)
- [buttonEmits](variables/buttonEmits.md)
- [buttonProps](variables/buttonProps.md)
- [COMPONENT\_SIZE](variables/COMPONENT_SIZE.md)
- [default](variables/default.md)
- [GConfigProvider](variables/GConfigProvider.md)
- [version](variables/version.md)

## 函数

- [useConfig](functions/useConfig.md)
- [useNamespace](functions/useNamespace.md)
- [useSize](functions/useSize.md)

## 参考

### GButton

theme_renames_and_re_exports [GConfigProvider](variables/GConfigProvider.md)
